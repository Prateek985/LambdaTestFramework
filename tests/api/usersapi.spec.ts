...
test('@regression POST - create a user', async ({ apiHelper }) => {
...
//Test 4: Patch a user test + verify: AAA
//POST ---> userId --> PATCH(200) --> GET /userId -- verify(200)
test('@regression PATCH - update a user', async ({ apiHelper }) => {
    //create a user: POST
    let userResponse = await createUser(apiHelper);

    //update the user:
    let response = await apiHelper.Patch(`/public/v2/users/${userResponse.id}`, AUTH_HEADER);
    expect(response.status).toBe(200);
